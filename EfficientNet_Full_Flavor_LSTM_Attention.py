...
class Config:
    ...
    def get_paths(cls):
        ...
        paths = {
            ...
            'model': os.path.join(cls.DATASET_PATH, f'best_model_efficientnet_{variant}_h{cls.HIDDEN_SIZE}.pth'),
            'tensorboard': os.path.join(cls.DATASET_PATH, 'runs', f'efficientnet_{variant}')
        }
        return paths
    ...
    def print_config(cls):
        ...
        print("="*70)
        print(f"EFFICIENTNET-{spec.name.upper()} CONFIGURATION")
        print("="*70)
        ...
        print(f"Min Word Freq: {cls.MIN_WORD_FREQ}")
        print("-"*70)
        ...
        print("-"*70)
        print(f"Features Path: {paths['features']}")
...
# ============================================================================
# ATTENTION - Automatically adapts to feature_dim
# ============================================================================
class Attention(nn.Module):
    def __init__(self, feature_dim, hidden_dim=Config.HIDDEN_SIZE, attention_dim=Config.ATTENTION_DIM):
        super(Attention, self).__init__()
        self.W_feat = nn.Linear(feature_dim, attention_dim)
        self.W_hidden = nn.Linear(hidden_dim, attention_dim)
        self.V = nn.Linear(attention_dim, 1)
    
    def forward(self, features, hidden):
        feat_proj = self.W_feat(features)
        hid_proj = self.W_hidden(hidden).unsqueeze(1)
        e = torch.tanh(feat_proj + hid_proj)
...
class ImageCaptioningModel(nn.Module):
    def __init__(self, vocab_size, feature_dim, embed_size=Config.EMBED_SIZE, 
                 hidden_size=Config.HIDDEN_SIZE, attention_dim=Config.ATTENTION_DIM,
                 embed_dropout=Config.EMBED_DROPOUT, lstm_dropout=Config.LSTM_DROPOUT,
...
class Trainer:
    ...
    def train_epoch(self, epoch):
        ...
        if self.config.SCHEDULED_SAMPLING:
            sampling_prob = get_sampling_probability(
                epoch, self.config.EPOCHS, self.config.SS_START_EPOCH,
                self.config.SS_MAX_RATIO, self.config.SS_STRATEGY
            )
        else:
            sampling_prob = 0.0
        
        pbar = tqdm(self.train_loader, desc=f"Epoch {epoch+1}/{self.config.EPOCHS} [Train] SS={sampling_prob:.3f}")
    ...
    def _forward_with_scheduled_sampling(self, image_features, input_seqs, target_seqs, lengths, sampling_prob):
        ...
        current_input = input_seqs[:, 0]
        
        for t in range(max_len):
            context, alpha = self.model.attention(image_features, h)
            embeddings = self.model.embedding(current_input)
            ...
            if t < max_len - 1:
                ...
                if use_prediction:
                    current_input = logits.argmax(dim=1).detach()
                else:
                    current_input = input_seqs[:, t + 1]
...
def main():
    ...
    model = ImageCaptioningModel(
        ...
        feature_dim=spec.feature_dim,  # Automatically use correct dimension
